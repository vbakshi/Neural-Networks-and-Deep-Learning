...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3072, hidden_dim=200, num_classes=10, reg=0.0, weight_scale=1e-2):
        ...
        self.reg = reg
        self.velocity = [0]*(len(self.layer1.params) + len(self.layer2.params))
    ...
    def loss(self, X, y):
        ...
        ###################################################
        dX = self.layer1.backward(self.layer2.backward(dout))
    ...
    def predict(self, X):
        ...
        a =  self.layer1.feedforward(X)
        
        b = self.layer2.feedforward(a)
        b = b - np.max(b,axis = 1).reshape(-1,1)
        
        soft = np.exp(b)/np.sum(np.exp(b),axis = 1).reshape(-1,1)
